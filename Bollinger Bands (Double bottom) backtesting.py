...
env = TradingEnv(balance_amount=100, balance_unit='GBP', trading_fee_multiplier=0.99925, symbols=symbols)

for i in range(len(df)):
    if env.balance_unit == 'GBP':

        for symbol in symbols:
            if env.bottoms[symbol] == 'hit' and df[f'{symbol}-GBP_Low'].iloc[i] > df[f'{symbol}_lower_band'].iloc[i]:
    ...
    if env.balance_unit != 'GBP':
        if env.tops[env.balance_unit] == 'hit' and (
                df[f'{env.balance_unit}-GBP_High'].iloc[i] < df[f'{env.balance_unit}_upper_band'].iloc[i]):
