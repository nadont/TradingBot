...
def plot_initial(plot_df, symbol):
    fig = go.Figure(data=[go.Candlestick(x=plot_df['OpenTime'],
                                         open=plot_df[f'{symbol}-GBP_Open'],
                                         high=plot_df[f'{symbol}-GBP_High'],
                                         low=plot_df[f'{symbol}-GBP_Low'],
                                         close=plot_df[f'{symbol}-GBP_Close'])])
...
def plot_results(plot_df, symbol, buys, sells):
    ...
    fig.update_yaxes(range=[min(plot_df[f'{symbol}-GBP_Low']) * .99, max(plot_df[f'{symbol}-GBP_High']) * 1.01])
